...
class UpdatePage(QWidget):
    def __init__(self, parent=None):
        ...
        self._current_channel = "github"  # github / gitee
    ...
    def setup_ui(self):
        ...
        channel_btn_layout.setSpacing(15)
        channel_btn_layout.setContentsMargins(0, 0, 0, 0)
        channel_btn_layout.addStretch()

        self.github_channel_btn = QPushButton("GitHub源")
        self.github_channel_btn.setFixedSize(100, 28)
        self.github_channel_btn.setCheckable(True)
        self.github_channel_btn.setStyleSheet("""
            QPushButton {
                font-size: 12px;
        ...
        """)
        self.github_channel_btn.clicked.connect(lambda: self._on_channel_changed("github"))
        channel_btn_layout.addWidget(self.github_channel_btn)

        self.gitee_channel_btn = QPushButton("Gitee源")
        self.gitee_channel_btn.setFixedSize(100, 28)
        self.gitee_channel_btn.setCheckable(True)
        self.gitee_channel_btn.setStyleSheet("""
            QPushButton {
                font-size: 12px;
                border: 1px solid #ccc;
                border-radius: 4px;
        ...
        """)
        self.gitee_channel_btn.clicked.connect(lambda: self._on_channel_changed("gitee"))
        channel_btn_layout.addWidget(self.gitee_channel_btn)
    ...
    def load_channel_setting(self):
        ...
        channel = settings.value("update_channel", "github")
        if channel == "gitee":
            self._current_channel = "gitee"
            self.gitee_channel_btn.setChecked(True)
            self.github_channel_btn.setChecked(False)
            self.token_edit.setEnabled(False)
        else:
            self._current_channel = "github"
            self.github_channel_btn.setChecked(True)
            self.gitee_channel_btn.setChecked(False)
    ...
    def _on_channel_changed(self, channel):
        ...
        if channel == "github":
            ...
            self.gitee_channel_btn.setChecked(False)
            self.token_edit.setEnabled(True)
            self.update_status_label.setText("已切换到 GitHub 源")
        else:
            self.gitee_channel_btn.setChecked(True)
            self.github_channel_btn.setChecked(False)
            self.token_edit.setEnabled(False)
            self.update_status_label.setText("已切换到 Gitee 源")
