...
class WeatherMixin:
    ...
    def start_weather_thread(self, force_restart=False):
        ...
        if service and service != "open_meteo":
            api_key = settings.value("api_key", "") or settings.value(f"api_key_{service}", "")
        else:
        ...
        if weather_service == "open_meteo":
            has_lat = settings.value("selected_latitude", "")
            has_lng = settings.value("selected_longitude", "")
            self._api_configured = bool(has_lat and has_lng)
        else:
